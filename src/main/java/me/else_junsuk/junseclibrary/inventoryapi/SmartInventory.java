...
import me.else_junsuk.junseclibrary.inventoryapi.content.InventoryProvider;
import me.else_junsuk.junseclibrary.inventoryapi.opener.InventoryOpener;
    ...
    public Inventory open(Player player, int page) {
        ...
        try {
            ...
            InventoryOpener opener = this.manager.findOpener(type)
                    .orElseThrow(() -> new IllegalStateException("인벤토리 유형(Type)에 대한 오프너(Opener)를 찾을 수 없습니다. " + type.name()));
    ...
    public static final class Builder {
        ...
        public SmartInventory build() {
            if(this.provider == null)
                throw new IllegalStateException("SmartInventory.Builder의 공급자(Provider)를 설정해야 합니다.");

            InventoryManager manager = this.manager != null ? this.manager : JunSecLibrary.manager();

            if(manager == null)
                throw new IllegalStateException("SmartInventory.Builder의 관리자를 설정하거나, "
                        + "JunSecLibrary를 플러그인으로 로드해야 합니다.");
